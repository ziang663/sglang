...
def _is_2604b_from_swiglu(swiglu_limit) -> bool:
    """Treat a configured SwiGLU clamp as DeepSeek V4 2604B.

    The upstream ktransformers integration keyed this off
    SGLANG_DSV4_2604_SUBMODE. Official SGLang 0.5.12 already carries
    swiglu_limit in the Hugging Face config, so relying on the model config
    keeps this repo runnable without extra user environment variables.
    """

    return swiglu_limit is not None or envs.SGLANG_DSV4_2604_SUBMODE.get() == "2604B"
...
class DeepSeekMxfp4MoEMethod:
    ...
    def create_moe_runner(self, layer, moe_runner_config):
        ...
        is_2604b = _is_2604b_from_swiglu(swiglu_limit)
        ...
        if getattr(layer, "_v4_tk_path", False):
            ...
            if not envs.SGLANG_OPT_MXFP4_FUSE_RSF_SHARED_ADD.get():
                if rsf is not None and rsf != 1.0:
                    output.mul_(rsf)
            if _is_2604b_from_swiglu(layer.moe_runner_config.swiglu_limit) and (
